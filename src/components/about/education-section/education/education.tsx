          ...
          <div key={idx} className="ed-item">
            ...
            <div className="ed-header">
              ...
              <div className="ed-icons-wrapper">
                {edu.degreeIcon && <i className={`ed-main-icon ${edu.degreeIcon}`}></i>}
                {edu.lewagonIcon && <i className={`ed-main-icon ${edu.lewagonIcon}`}></i>}
              </div>

              {/* Degree + specialization + school + dates */}
              <div className="ed-degree-block">
                <h2 className="ed-degree">{edu.degree}</h2>
                {edu.specialization && <p className="ed-specialization">{edu.specialization}</p>}

                <div className="ed-school-row">
                  {edu.degreeIcon && <i className={`ed-school-icon ${edu.degreeIcon}`}></i>}
                  <span className="ed-school">{edu.school}</span>
                  <span className="ed-dates">
                    {edu.icons?.startDate && <i className={edu.icons.startDate}></i>}{" "}
              ...
              {/* Optional emblem */}
              {edu.emblem && <img src={edu.emblem} alt={`${edu.school} emblem`} className="ed-emblem" />}
            </div>

            {/* Description */}
            {edu.description && <p className="ed-description">{edu.description}</p>}

            {/* Key content */}
            {edu.keyContent && edu.keyContent.length > 0 && (
              <>
                {edu.keyContentTitle && <h3 className="ed-keycontent-title">{edu.keyContentTitle}</h3>}
